        ...
        <div className="max-w-5xl mx-auto">
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-14">
            {contactInfo.map((item, i) => (
              <motion.div key={item.label} {...fadeUp(i * 0.08)}
                className="bg-white dark:bg-neutral-700 rounded-2xl p-5 shadow-sm border border-neutral-100 dark:border-neutral-600 text-center hover:shadow-md transition-shadow">
                ...
                <p className="text-xs font-semibold text-neutral-400 dark:text-neutral-400 uppercase tracking-wide mb-1">{item.label}</p>
          ...
          <div className="grid lg:grid-cols-2 gap-10 items-start">
            <motion.div {...fadeUp(0.1)}>
              ...
              <p className="text-neutral-500 dark:text-neutral-400 mb-6">Fill in the form and we'll respond within 24 hours.</p>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input {...register('name', { required: 'Name is required' })}
                      className="w-full px-4 py-3 border-2 border-neutral-200 dark:border-neutral-600 dark:bg-neutral-700 dark:text-white rounded-xl focus:outline-none focus:border-emerald-500 transition-all text-sm"
                ...
                <div>
                  ...
                  <textarea {...register('message', { required: 'Message is required', minLength: { value: 20, message: 'Please write at least 20 characters' } })}
                    rows={5}
                    className="w-full px-4 py-3 border-2 border-neutral-200 dark:border-neutral-600 dark:bg-neutral-700 dark:text-white rounded-xl focus:outline-none focus:border-emerald-500 transition-all text-sm resize-none"
            ...
            <motion.div {...fadeUp(0.2)} className="space-y-6">
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-6">
                <h3 className="text-lg font-bold text-red-700 dark:text-red-400 mb-2">🆘 Crisis Support</h3>
                <p className="text-sm text-red-600 dark:text-red-300 leading-relaxed mb-4">If you or someone you know is in immediate danger, please contact emergency services immediately.</p>
                <div className="space-y-2">
                  {[{ label: 'Nigeria Emergency', number: '112' }, { label: 'Suicide Prevention (Nigeria)', number: '0800-800-2000' }, { label: 'MentWel Crisis Line', number: '+234 800 MENTWEL' }].map((line) => (
                    <div key={line.label} className="flex items-center justify-between bg-white dark:bg-neutral-800 rounded-xl px-4 py-2.5 border border-red-100 dark:border-red-900/30">
                      <span className="text-sm text-neutral-600 dark:text-neutral-300">{line.label}</span>
                      <span className="text-sm font-bold text-red-600 dark:text-red-400">{line.number}</span>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-sky-50 dark:bg-sky-900/20 border border-sky-200 dark:border-sky-800 rounded-2xl p-6">
                <h3 className="text-lg font-bold text-sky-700 dark:text-sky-400 mb-2">💬 Live Chat</h3>
                <p className="text-sm text-sky-600 dark:text-sky-300 leading-relaxed mb-4">Need a faster response? Our support team is available via live chat during business hours.</p>
                <div className="flex items-center gap-2 text-sm text-sky-600 dark:text-sky-400">
                  <Clock className="w-4 h-4" /><span>Mon – Fri, 8am – 8pm WAT</span>
                </div>
              </div>
            </motion.div>
          </div>
